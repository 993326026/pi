...
- `name` 是必需的，并且必须唯一。
- `vars` 是可选的。在此处定义可重用颜色，然后在 `colors` 中引用它们。
...
- `0-15`：基本 ANSI 颜色（终端相关）
- `16-231`：6×6×6 RGB 立方体（`16 + 36×R + 6×G + B`，其中 R、G、B 为 0-5）
- `232-255`：灰度渐变

### 终端兼容性

Pi 使用 24 位 RGB 颜色。大多数现代终端支持此功能（iTerm2、Kitty、WezTerm、Windows Terminal、VS Code）。对于仅支持 256 色的较旧终端，pi 会回退到最近的近似值。
...
**深色终端**：使用具有更高对比度的明亮、饱和颜色。

**浅色终端**：使用具有更低对比度的更深、更柔和颜色。

**颜色和谐**：从基础调色板（Nord、Gruvbox、Tokyo Night）开始，在 `vars` 中定义，并一致引用。

**测试**：使用不同的消息类型、工具状态、markdown 内容和长换行文本测试你的主题。

**VS Code**：将 `terminal.integrated.minimumContrastRatio` 设置为 `1` 以获得准确颜色。

## 示例

查看内置主题：
- [dark.json](https://github.com/earendil-works/pi-mono/blob/main/packages/coding-agent/src/modes/interactive/theme/dark.json)
- [light.json](https://github.com/earendil-works/pi-mono/blob/main/packages/coding-agent/src/modes/interactive/theme/light.json)
