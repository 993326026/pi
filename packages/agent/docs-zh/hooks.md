...
<!-- 从 jot 3utlzkxy 同步。在仓库中编辑此文件以继续。 -->

最终设计。

## 核心模型

事件将其结果类型作为仅类型的幽灵类型携带：
...
内部类型转换在实现中是可接受的，因为 `Map<string, ...>` 会失去特异性。公共 API 保持类型化。
...
观察者运行。`message_end` 处理程序运行。返回被忽略，除非该事件稍后获得结果类型。
...
Harness 不存储处理程序、链接监听器或知道扩展策略。

## 上下文

上下文是一个普通对象，不是每个 emit 重新构建的。
...
对于动态状态，更喜欢稳定的外观/方法而不是 getter 迷宫：
...
每次运行的 `signal` 作为第三个处理程序参数传递。

## 稍后的扩展加载

扩展加载可以位于 harness 旁边并构造钩子：
...
harness.setHooks(nextHooks); // 如果支持则仅空闲
```
...
现有的 coding-agent 捕获扩展错误，报告它们并继续。新钩子需要相同的策略，可能：
...
对于 coding-agent，默认应该是 `"continue"`。

### 2. 源元数据很重要

现有的运行器知道哪个扩展产生了错误/资源/工具。除非我们添加注册元数据或作用域，否则普通的 `on()` 会失去这个。
...
这些不被 `emit()` 覆盖，并且应该作为 `CodingAgentHooks` 或扩展主机上的注册表保留：
...
这很好。它们不属于 `AgentHarness`。

### 4. 现有的 coding-agent 事件可以被表示
...
移植 coding-agent 时，必须复制特殊情况：

- `input`：转换链，`handled` 短路。
- `user_bash`：第一个有意义的结果获胜。
- `message_end`：替换必须保持相同的角色。
- `before_agent_start`：`ctx.getSystemPrompt()` 必须反映当前链接的提示。
- `resources_discover`：聚合路径并保留扩展源。
- `tool_call`：参数变更对后续处理程序保持可见。
- `tool_result`：后续处理程序看到之前的补丁。

设计允许所有这些，但默认/coding 钩子实现必须编码它。

### 6. `emit()` 开关可能会错过自定义变更事件

如果子类添加产生结果的事件但忘记覆盖 `emit()`，它将表现得像观察性的。测试应该捕获这个。如果这变得容易出错，可以稍后添加受保护的策略注册表，但最初不添加。

### 7. 观察者语义故意受限

观察者一次看到原始发出的事件。它们看不到每个中间变更。如果某些东西需要最终转换的状态，发出一个单独的最终事件或使用事件特定的处理程序。

## 结论

此设计可以实现一个新的 coding-agent。它比当前的运行器更简单，保持 harness 清洁，并且只要 `CodingAgentHooks` 添加源感知作用域、注册表、清理和精确的旧事件语义，就保留重要的扩展能力。

--- 评论 ---

关于 "addCleanup(cleanup" 的线程 hn2xk0tzhj
  [tmluyaub9v] 所有者 (2026-05-14T12:55:45.500Z)：清理应该可选地传递给 on/observe
